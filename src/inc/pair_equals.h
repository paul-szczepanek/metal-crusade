...
template <class T1, class T2> struct PairFirstEquals {
  PairFirstEquals (const T1& a_value)
    : value_first(a_value) {
  }
  ...
  bool operator() (pair<T1, T2> a_value) {
    return value_first == a_value.first;
  }

};

template <class T1, class T2> struct PairSecondEquals {
  PairSecondEquals (const T2& a_value)
    : value_second(a_value) {
  }
  ...
  // comparison for stl find_if
  bool operator() (pair<T1, T2> a_value) {
    return value_second == a_value.second;
  }
