...
static FilesHandler* Inst = 0;

void FilesHandler::init()
{
                                                      ...
                                                      string     a_buffer)
{
  ...
  for(uint i = 0, for_size = string_array.size(); i < for_size; ++i) {
...
{
  if(!a_string.empty()) {
    uint begin = a_string.find_first_not_of(" ");
    uint end = a_string.find_last_not_of(" ");
                                 ...
                                 string        a_buffer)
{
  ...
  string value;
  stripWhitespace(a_buffer);
  uint open_brace = a_buffer.find_first_of("(");
  uint close_brace = a_buffer.find_first_of(")");
                                   ...
                                   string            a_buffer)
{
  ...
  for(uint i = 0; i < string_array.size() - 1; ++i) {
