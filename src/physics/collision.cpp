...
static const Real repulsion = 2;
static const Real damping = 0.9;
...
{
  const Real collision_elasticity = 0.2; // collision soft/hard ratio maybe parametrise later
  Real weight1 = Object1->getWeight();
  Real weight2 = Object2->getWeight();
  Real weight_denominator = 1 / (weight1 + weight2);
  ...
  Vector3 combined_velocity = ((weight1 * Velocity1)
  ...
  if (ResultType == collision_type_impact) {
    ...
    Vector3 collision_vector = (sphere1.Centre - sphere2.Centre);
    ...
    if (ResultType == collision_type_blocking) {
      ...
      Real velocity1_perp = Velocity1.dotProduct(collision_vector);
      Real velocity2_perp = Velocity2.dotProduct(collision_vector);

      // scalar velocities after collision
      Real new_velocity1_perp = ((velocity1_perp * (weight1 - weight2)
                                  + 2 * weight2 * velocity2_perp) * weight_denominator);
      Real new_velocity2_perp = ((velocity2_perp * (weight2 - weight1)
    ...
    Real depth = sphere1.distanceToSurface(sphere2); // how deep the hit is
  ...
  if (ResultType != collision_type_impact) { // if it's an impact heat damage is handled in object
    // get surface temps
    Real surface_temperature1 = Object1->getSurfaceTemperature();
    Real surface_temperature2 = Object2->getSurfaceTemperature();
